...
resource "azurerm_role_assignment" "general_role_assignment" {
  ...
  scope                                  = each.value.general_role_assignment_target_resource_type == "ManagementGroup" ? "/subscriptions/${data.azurerm_subscription.current.display_name}/Microsoft.Management/managementGroups/${data.azurerm_management_group.management_group_id[each.key].id}" : (each.value.general_role_assignment_target_resource_type == "Subscription" ? "/subscriptions/${each.value.general_role_assignment_target_resource_name}" : "${data.azurerm_resources.azure_resource[each.key].resources[0].id}")
