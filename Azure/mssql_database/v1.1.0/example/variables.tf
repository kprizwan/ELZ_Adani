...
variable "mssql_database_variables" {
  type = map(object({
    ...
    mssql_database_threat_detection_policy = object({
      ...
      threat_detection_policy_disabled_alerts      = list(string)
      threat_detection_policy_email_account_admins = string
      threat_detection_policy_email_addresses      = list(string)
      threat_detection_policy_retention_days       = number
    })
    mssql_database_long_term_retention_policy_required = bool
    mssql_database_long_term_retention_policy = object({
      long_term_retention_policy_weekly_retention  = string
      long_term_retention_policy_monthly_retention = string
      long_term_retention_policy_yearly_retention  = string
      long_term_retention_policy_week_of_year      = number
    })
    mssql_database_short_term_retention_policy_required = bool
    mssql_database_short_term_retention_policy = object({
      short_term_retention_policy_retention_days           = number
