...
variable "eventhub_namespace_variables" {
  type = map(object({
    ...
    eventhub_namespace_identity = object({                             # (Optional) An identity block
      eventhub_namespace_identity_type = string                        # (Required) Specifies the type of Managed Service Identity that should be configured on this Data Factory. Possible values are SystemAssigned, UserAssigned, SystemAssigned, UserAssigned (to enable both).
      eventhub_namespace_user_assigned_identities = list(object({      # (Optional) Specifies a list of User Assigned Managed Identity IDs to be assigned to this Data Factory.
