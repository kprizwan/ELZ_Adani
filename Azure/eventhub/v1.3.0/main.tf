#DATA BLOCK TO FETCH STORAGE ACCOUNT ID
...
resource "azurerm_eventhub" "eventhub" {
  ...
  dynamic "capture_description" {
    ...
    content {
      ...
      dynamic "destination" {
        for_each = capture_description.value.capture_description_enabled == true ? [capture_description.value.capture_description_destination] : []
        content {
