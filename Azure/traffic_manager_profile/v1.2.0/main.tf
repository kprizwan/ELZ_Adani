resource "azurerm_traffic_manager_profile" "traffic_manager_profile" {
  ...
  dns_config {
    relative_name = each.value.traffic_manager_profile_dns_config.dns_config_relative_name
    ttl           = each.value.traffic_manager_profile_dns_config.dns_config_ttl
  }
  monitor_config {
    ...
    dynamic "custom_header" {
      for_each = each.value.traffic_manager_profile_monitor_config.monitor_config_custom_header != null ? toset(each.value.traffic_manager_profile_monitor_config.monitor_config_custom_header) : []
      content {
