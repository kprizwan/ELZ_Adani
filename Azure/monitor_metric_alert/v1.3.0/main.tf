...
#DATA BLOCK FOR MONITOR ACTION GROUP
data "azurerm_monitor_action_group" "monitor_action_group" {
  provider            = azurerm.monitor_metric_alert_sub
  for_each            = { for k, v in var.monitor_metric_alert_variables : k => v if v.monitor_metric_alert_action != null }
  resource_group_name = each.value.monitor_action_group_resource_group_name
  name                = each.value.monitor_action_group_name
}
...
resource "azurerm_monitor_metric_alert" "monitor_metric_alert" {
  ...
  dynamic "action" {
    for_each = each.value.monitor_metric_alert_action == null ? [] : toset(each.value.monitor_metric_alert_action)
    content {
      action_group_id    = each.value.monitor_metric_alert_action != null ? data.azurerm_monitor_action_group.monitor_action_group[each.key].id : null
