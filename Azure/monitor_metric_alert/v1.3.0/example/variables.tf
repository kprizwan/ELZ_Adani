...
#MONITOR ACTION GROUP VARIABLES
variable "monitor_action_group_variables" {
  description = "Map of Monitor Action Group Variables"
  type = map(object({
    monitor_action_group_name                = string      #(Required) The name of the Action Group.
    monitor_action_group_resource_group_name = string      #(Required) The name of the resource group in which to create the Action Group instance.
    monitor_action_group_location            = string      #(Optional) The Azure Region where the Action Group should exist. Changing this forces a new Action Group to be created. Defaults to global.
    monitor_action_group_short_name          = string      #(Required) The short name of the action group. This will be used in SMS messages.
    monitor_action_group_enabled             = bool        #(Optional) Whether this action group is enabled. If an action group is not enabled, then none of its receivers will receive communications. Defaults to true.
    monitor_action_group_tags                = map(string) #(Optional) A mapping of tags to assign to the resource. 
    monitor_action_group_arm_role_receiver = list(object({ #(Optional) One or more arm_role_receiver blocks as defined below
      arm_role_receiver_name                    = string   #(Required) The name of the ARM role receiver.
      arm_role_receiver_role_id                 = string   #(Required) The arm role id.
      arm_role_receiver_use_common_alert_schema = bool     #(Optional) Enables or disables the common alert schema.
    }))
    monitor_action_group_automation_runbook_receiver = list(object({   #(Optional) One or more automation_runbook_receiver blocks as defined below.
    ...
    monitor_action_group_azure_app_push_receiver = list(object({ #(Optional) One or more azure_app_push_receiver blocks as defined below.
      azure_app_push_receiver_name          = string             #(Required) The name of the Azure app push receiver.
      azure_app_push_receiver_email_address = string             #(Required) The email address of the user signed into the mobile app who will receive push notifications from this receiver.
    }))
    monitor_action_group_azure_function_receiver = list(object({ #(Optional) One or more azure_function_receiver blocks as defined below.
      monitor_action_group_function_receiver_name  = string      #(Required) The name of the Azure Function receiver. 
      monitor_action_group_function_name           = string      #(Required) The function name in the function app.
      monitor_action_group_http_trigger_url        = string      #(Required) The HTTP trigger url where HTTP request sent to.
      monitor_action_group_use_common_alert_schema = bool        #(Optional) Enables or disables the common alert schema. 
    }))
    monitor_action_group_email_receiver = list(object({ #(Optional) One or more email_receiver blocks as defined below.
      email_receiver_name                    = string   #(Required) The name of the email receiver. Names must be unique (case-insensitive) across all receivers within an action group.
      email_receiver_email_address           = string   #(Required) The email address of this receiver.
      email_receiver_use_common_alert_schema = bool     #(Optional) Enables or disables the common alert schema. 
    }))
    monitor_action_group_event_hub_receiver = list(object({ #(Optional) One or more event_hub_receiver blocks as defined below.
    ...
    monitor_action_group_itsm_receiver = list(object({ #(Optional) One or more itsm_receiver blocks as defined below.
      itsm_receiver_name                      = string #(Required) The name of the ITSM receiver.
      itsm_receiver_name_connection_id        = string #(Required) The unique connection identifier of the ITSM connection.
      itsm_receiver_name_ticket_configuration = string #(Required) A JSON blob for the configurations of the ITSM action.
      itsm_receiver_name_region               = string #(Required) The region of the workspace.
    }))
    monitor_action_group_logic_app_receiver = list(object({ #(Optional) One or more logic_app_receiver blocks as defined below.
    ...
    monitor_action_group_sms_receiver = list(object({ #(Optional) One or more sms_receiver blocks as defined below.
    ...
    monitor_action_group_voice_receiver = list(object({ #(Optional) One or more voice_receiver blocks as defined below.
    ...
    monitor_action_group_webhook_receiver = map(object({ #(Optional) One or more webhook_receiver blocks as defined below
      ...
      webhook_receiver_aad_auth = object({               #(Optional) The aad_auth block as defined below
        aad_auth_identifier_uri = string                 #(Optional) The identifier URI for AAD auth.
      })
    }))
    monitor_action_group_itsm_receiver_log_analytics_workspace_name                         = string #(Optional) Specifies the name of the Log Analytics Workspace. Workspace name should include 4-63 letters, digits or '-'. The '-' shouldn't be the first or the last symbol. Changing this forces a new resource to be created.
    monitor_action_group_itsm_receiver_log_analytics_workspace_resource_group_name          = string #(Optional) The name of the resource group in which the Log Analytics workspace is created. Changing this forces a new resource to be created.
    monitor_action_group_logic_app_receiver_logic_app_workflow_name                         = string #(Optional) The name of the Logic App Workflow.
    monitor_action_group_logic_app_receiver_logic_app_workflow_resource_group_name          = string #(Optional) The name of the Resource Group in which the Logic App Workflow exists.
    monitor_action_group_automation_runbook_receiver_automation_account_name                = string #(Optional) The name of the Automation Account.
    monitor_action_group_automation_runbook_receiver_automation_account_resource_group_name = string #(Optional) Specifies the name of the Resource Group where the Automation Account exists.
    monitor_action_group_function_app_function_os_type                                      = string #(Required) Function app OS type. Possible values are windows and linux.
    monitor_action_group_azure_function_receiver_linux_function_name                        = string #(Optional) The function name in the function app.
    monitor_action_group_azure_function_receiver_linux_function_app_resource_group_name     = string #(Optional) The name of the resource group in which to create the Action Group instance.
    monitor_action_group_azure_function_receiver_windows_function_name                      = string #(Optional) The function name in the function app.
    monitor_action_group_azure_function_receiver_windows_function_app_resource_group_name   = string #(Optional) The name of the resource group in which to create the Action Group instance.
  }))
  default = {}
}
