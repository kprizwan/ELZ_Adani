...
data "azurerm_key_vault_secret" "secret_connection_string_value" {
  provider     = azurerm.key_vault_sub
  for_each     = local.is_key_vault_exists
  name         = each.value.secret_connection_string_value_name
...
resource "azurerm_logic_app_standard" "logic_app_standard" {
  ...
  location                   = each.value.logic_app_standard_resource_group_location
  resource_group_name        = each.value.logic_app_standard_resource_group_name
  ...
  dynamic "connection_string" {
    ...
    content {
      ...
      value = data.azurerm_key_vault_secret.secret_connection_string_value[each.key].value
    }
  }
  ...
  dynamic "identity" {
    for_each = each.value.logic_app_standard_identity != null ? [each.value.logic_app_standard_identity] : []
    content {
