...
storage_account_variables = {
  "storage_account_01" = {
    storage_account_key_vault_name                                     = "ploceuskv000001"     #(Required) The name of the Key Vault.
...
service_plan_variables = {
  "service_plan_1" = {
    ...
    service_plan_location                     = "eastus"             #(Required) The Azure Region where the Service Plan should exist. Changing this forces a new AppService to be created.
    ...
    service_plan_sku_name                     = "WS1"                #(Required) The SKU for the plan. Possible values include B1, B2, B3, D1, F1, I1, I2, I3, I1v2, I2v2, I3v2, P1v2, P2v2, P3v2, P1v3, P2v3, P3v3, S1, S2, S3, SHARED, EP1, EP2, EP3, WS1, WS2, WS3, and Y1.
...
logic_app_standard_variables = {
  "logic_app_standard_variables_1" = {
    ...
    logic_app_standard_storage_account_resource_group_name = "ploceusrg000003"        #(Required) The resource group name of the storage account.
    logic_app_standard_service_plan_resource_group_name    = "ploceusrg000001"        #(Required) The resource group name of the app service plan.
    ...
    logic_app_standard_service_plan_name                   = "ploceussplan000001"     #(Required) The name of the app service plan.
    ...
    logic_app_standard_connection_string                   = null /* {                                           #(Optional)
      connection_string_name = "ploceuslaconnstring000001"                                               #(Required) The name of the Connection String.
      connection_string_type = "SQLServer"                                                               #(Required)Possible values are APIHub, Custom, DocDb, EventHub, MySQL, NotificationHub, PostgreSQL, RedisCache, ServiceBus, SQLAzure and SQLServer.
    }*/
    ...
    logic_app_standard_identity = {                                #(Optional)
      ...
      logic_app_standard_user_assigned_identities = [{                            #Optional) Specifies a list of User Assigned Managed Identity IDs to be assigned to this Logic App Standard.
        logic_app_standard_user_identity_name                = "ploceusuai000001" # (Optional) Specifies a User Assigned Managed Identity Name to be assigned to this Logic App standard.
    ...
    logic_app_standard_site_config                         = null /*[{ #(Optional)
      site_config_always_on       = false           #(Optional) Should the Logic App be loaded at all times? Defaults to false.
      site_config_app_scale_limit = null            #(Optional) The number of workers this Logic App can scale out to. Only applicable to apps on the Consumption and Premium plan.
      site_config_cors = [{                         #(Optional)
        cors_allowed_origins     = ["*"]     #(Optional) A list of origins which should be able to make cross-origin calls. * can be used to allow all calls.
        cors_support_credentials = false     #(Optional) Are credentials supported?
      }]
      site_config_dotnet_framework_version         = null  #(Optional) The version of the .NET framework's CLR used in this Logic App Possible values are v4.0 (including .NET Core 2.1 and 3.1), v5.0 and v6.0. For more information on which .NET Framework version to use based on the runtime version you're targeting - please see this table. Defaults to v4.0.
      site_config_elastic_instance_minimum         = null  #(Optional) The number of minimum instances for this Logic App Only affects apps on the Premium plan.
      site_config_ftps_state                       = null  #(Optional) State of FTP / FTPS service for this Logic App Possible values include: AllAllowed, FtpsOnly and Disabled. Defaults to AllAllowed.
      site_config_health_check_path                = null  #(Optional) Path which will be checked for this Logic App health.
      site_config_http2_enabled                    = false #(Optional) Specifies whether or not the HTTP2 protocol should be enabled. Defaults to false.
      site_config_ip_restriction                   = [{       #(Optional) 
        ip_restriction_ip_address  = null               #(Optional) The IP Address used for this IP Restriction in CIDR notation.
        ip_restriction_service_tag = null               #(Optional) The Service Tag used for this IP Restriction.
        ip_restriction_name        = null               #(Optional) The name for this IP Restriction.
        ip_restriction_priority    = null               # (Optional) The priority for this IP Restriction. Restrictions are enforced in priority order. By default, the priority is set to 65000 if not specified.
        ip_restriction_action      = null               #(Optional) Does this restriction Allow or Deny access for this IP range. Defaults to Allow.
        ip_restriction_headers = {                 #(Optional)
          headers_x_azure_fdid      = null #(Optional) A list of allowed Azure FrontDoor IDs in UUID notation with a maximum of 8.
          headers_x_fd_health_probe = null #(Optional) A list to allow the Azure FrontDoor health probe header. Only allowed value is "1".
          headers_x_forwarded_for   = null #(Optional) A list of allowed 'X-Forwarded-For' IPs in CIDR notation with a maximum of 8
          headers_x_forwarded_host  = null #(Optional) A list of allowed 'X-Forwarded-Host' domains with a maximum of 8.
        }
      }]
      site_config_linux_fx_version                 = null  #(Optional) Linux App Framework and version for the AppService, e.g. DOCKER|(golang:latest). Setting this value will also set the kind of application deployed to functionapp,linux,container,workflowapp
      site_config_min_tls_version                  = null  #(Optional) The minimum supported TLS version for the Logic App Possible values are 1.0, 1.1, and 1.2. Defaults to 1.2 for new Logic Apps.
      site_config_pre_warmed_instance_count        = null  #(Optional) The number of pre-warmed instances for this Logic App Only affects apps on the Premium plan.
      site_config_runtime_scale_monitoring_enabled = false #(Optional) Should Runtime Scale Monitoring be enabled?. Only applicable to apps on the Premium plan. Defaults to false.
      site_config_use_32_bit_worker_process        = true  #(Optional) Should the Logic App run in 32 bit mode, rather than 64 bit mode? Defaults to true.
      site_config_vnet_route_all_enabled           = false #(Optional) Should all outbound traffic to have Virtual Network Security Groups and User Defined Routes applied.
      site_config_websockets_enabled               = false #(Optional) Should WebSockets be enabled?
      site_config_scm_ip_restriction               = [{   #(Optional) 
        scm_ip_restriction_ip_address  = null               #(Optional) The IP Address used for this IP Restriction in CIDR notation.
        scm_ip_restriction_service_tag = null               #(Optional) The Service Tag used for this IP Restriction.
        scm_ip_restriction_name        = null               #(Optional) The name for this IP Restriction.
        scm_ip_restriction_priority    = null               #(Optional) The priority for this IP Restriction. Restrictions are enforced in priority order. By default, the priority is set to 65000 if not specified.
        scm_ip_restriction_action      = null               #(Optional) Does this restriction Allow or Deny access for this IP range. Defaults to Allow.
        scm_ip_restriction_headers = {                 #(Optional)
          headers_x_azure_fdid      = null #(Optional) A list of allowed Azure FrontDoor IDs in UUID notation with a maximum of 8.
          headers_x_fd_health_probe = null #(Optional) A list to allow the Azure FrontDoor health probe header. Only allowed value is "1".
          headers_x_forwarded_for   = null #(Optional) A list of allowed 'X-Forwarded-For' IPs in CIDR notation with a maximum of 8
          headers_x_forwarded_host  = null #(Optional) A list of allowed 'X-Forwarded-Host' domains with a maximum of 8.
      ...
      site_config_scm_use_main_ip_restriction      = false #(Optional) Should the Logic App ip_restriction configuration be used for the SCM too. Defaults to false.
      site_config_scm_min_tls_version              = null  #(Optional) Configures the minimum version of TLS required for SSL requests to the SCM site.
      site_config_scm_type                         = null  #The type of Source Control used by the Logic App in use by the Windows Function App. Defaults to None. Possible values are: BitbucketGit, BitbucketHg, CodePlexGit, CodePlexHg, Dropbox, ExternalGit, ExternalHg, GitHub, LocalGit, None, OneDrive, Tfs, VSO, and VSTSRM
    }]*/
    ...
    logic_app_standard_key_vault_name                      = "ploceuskv000001"         #(Required) The name of the key vault.
...
logic_app_standard_subscription_id = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"
logic_app_standard_tenant_id       = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"
key_vault_subscription_id          = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"
key_vault_tenant_id                = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"
storage_account_subscription_id    = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"
storage_account_tenant_id          = "xxxxxxxx-xxxxxxxx-xxxxxxxxxxx-xxxxxx"