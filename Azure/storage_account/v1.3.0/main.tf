locals {
  ...
  network_rule_subnet_ids = {
    for k, v in var.storage_account_variables : k => flatten([
...
resource "azurerm_storage_account" "storage_account" {
  ...
  allowed_copy_scope                = each.value.storage_account_allowed_copy_scope
  sftp_enabled                      = lookup(each.value, "storage_account_is_hns_enabled") == true ? each.value.storage_account_sftp_enabled : false
  ...
  dynamic "customer_managed_key" {
    ...
    content {
      key_vault_key_id          = each.value.storage_account_is_key_vault_key_versionless == true ? data.azurerm_key_vault_key.key_vault_key[each.key].versionless_id : data.azurerm_key_vault_key.key_vault_key[each.key].id
  ...
  dynamic "blob_properties" {
    ...
    content {
      ...
      dynamic "restore_policy" {
        ...
        content {
          days = restore_policy.value.restore_policy_days
        }
      }
  ...
  dynamic "network_rules" {
    ...
    content {
      ...
      virtual_network_subnet_ids = local.network_rule_subnet_ids[each.key]
