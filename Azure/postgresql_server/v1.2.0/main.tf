...
data "azurerm_storage_account" "storage_account" {
  ...
  for_each            = { for k, v in var.postgresql_server_variables : k => v if v.postgresql_server_threat_detection_policy.postgresql_server_enable_threat_detection_policy == true && v.postgresql_server_threat_detection_policy.postgresql_server_storage_account_name != null }
  name                = each.value.postgresql_server_threat_detection_policy.postgresql_server_storage_account_name
  resource_group_name = each.value.postgresql_server_threat_detection_policy.postgresql_server_storage_account_resource_group
}
...
data "azurerm_postgresql_server" "postgresql_server_id" {
  ...
  for_each            = { for k, v in var.postgresql_server_variables : k => v if v.postgresql_server_creation_source_server_name != null }
  name                = each.value.postgresql_server_creation_source_server_name
...
resource "azurerm_postgresql_server" "postgresql_server" {
  ...
  creation_source_server_id         = each.value.postgresql_server_creation_source_server_name == null ? null : data.azurerm_postgresql_server.postgresql_server_id[each.key].id
  ...
  dynamic "threat_detection_policy" {
    ...
    content {
      ...
      storage_account_access_key = each.value.postgresql_server_threat_detection_policy.postgresql_server_storage_account_name != null ? data.azurerm_storage_account.storage_account[each.key].primary_access_key : null
      storage_endpoint           = each.value.postgresql_server_threat_detection_policy.postgresql_server_storage_account_name != null ? data.azurerm_storage_account.storage_account[each.key].primary_blob_endpoint : null
    }
  }
