...
#DATA BLOCK TO FETCH USER ASSIGNED IDENTITY ID FOR ENCRYPTION
data "azurerm_user_assigned_identity" "user_assigned_identity_for_encryption" {
  provider            = azurerm.user_assigned_identity_sub
  for_each            = { for k, v in var.automation_account_variables : k => v if lookup(v, "automation_account_uid_cmk_encryption_name") != null }
...
data "azurerm_key_vault" "key_vault" {
  ...
  for_each            = { for k, v in var.automation_account_variables : k => v if lookup(v, "automation_account_key_vault_name") != null }
...
#DATA BLOCK TO FETCH KEYVAULT KEY ID
data "azurerm_key_vault_key" "encryption_key" {
  provider = azurerm.key_vault_sub
  for_each = { for k, v in var.automation_account_variables : k => v if lookup(v, "automation_account_key_vault_key_name") != null }
...
resource "azurerm_automation_account" "automation_account" {
  ...
  dynamic "encryption" {
    for_each = each.value.automation_account_key_vault_key_name != null ? [1] : []
    content {
